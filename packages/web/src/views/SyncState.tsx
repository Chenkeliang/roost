...
function Pill({ text, color }: { text: string; color: string }) {
  return (
    <span style={{ fontSize: 10.5, fontWeight: 600, padding: "2px 8px", borderRadius: 999, color, border: `1px solid ${color}` }}>
      {text}
    </span>
  );
}
...
function DiffPane({ label, text, accent }: { label: string; text: string | null; accent: string }) {
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, border: `1px solid ${accent}`, borderRadius: 8, overflow: "hidden" }}>
      <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: ".05em", color: "var(--muted)", padding: "4px 8px", borderBottom: "1px solid var(--border-soft)" }}>
        {label}
      </div>
      <pre
        style={{
      ...
      >
        ...
        {item.exception === "blocked" && onOpenSettings ? (
          <button
            ...
            style={{ fontSize: 10.5, fontWeight: 600, padding: "3px 11px", borderRadius: 7, cursor: "pointer", background: "transparent", border: "1px solid #b79af0", color: "#b79af0" }}
          >
            {t("sync.goSettings")} ▸
          </button>
        ) : null}
      ...
      {open ? (
        <div style={{ padding: "0 14px 12px", borderBottom: "1px solid var(--border-soft)" }}>
          {diffErr ? (
            ...
            <div style={{ color: "var(--muted)", fontSize: 12 }}>{t("sync.loadingDiff")}</div>
          ) : diff.kind === "summary" ? (
            <div style={{ color: "var(--muted)", fontSize: 12 }}>{summaryText(diff.summary)}</div>
            ...
            <div style={{ border: "1px solid var(--border-soft)", borderRadius: 8, overflow: "hidden" }}>
              ...
              <div style={{ fontSize: 10.5, color: "var(--muted)", padding: "5px 10px" }}>{t("sync.perKeyNote")}</div>
            </div>
          ) : (
...
function Group({
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, letterSpacing: ".05em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 600, margin: "0 0 6px" }}>
...
export function SyncState({ onOpenSettings }: { onOpenSettings?: () => void } = {}) {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: "0 auto", padding: "0 24px" }}>
      ...
      {loading && !data ? (
        <div style={{ color: "var(--muted)", fontSize: 13 }}>{t("sync.loading")}</div>
      ) : error ? (
      ...
      {notice ? (
        <div style={{ padding: "8px 14px", background: "var(--surface)", border: "1px solid var(--border-soft)", borderRadius: "var(--rc)", color: "#5fd08a", fontSize: 12.5, marginBottom: 12 }}>
          {notice}
        </div>
      ) : null}
      ...
      {data ? (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", background: "var(--surface)", border: "1px solid var(--border-soft)", borderRadius: "var(--rc)", marginBottom: 16, fontSize: 12.5 }}>
              ...
              {POLICY_OPTS.map((o) => {
                ...
                return (
                  <button
                    ...
                    style={{ appearance: "none", border: 0, background: active ? "var(--raise)" : "transparent", color: active ? "var(--text)" : "var(--muted)", fontSize: 11.5, padding: "3px 11px", cursor: "pointer" }}
                  >
                    {t(o.labelKey)}
                  </button>
                );
            ...
            {diverged.length + destructive.length > 0 ? (
              <button
                onClick={runBatch}
                disabled={batching}
                style={{ fontSize: 11.5, fontWeight: 700, padding: "5px 13px", borderRadius: 8, cursor: batching ? "default" : "pointer", background: "var(--accent)", border: "1px solid var(--accent)", color: "#1b1b1e" }}
              >
          ...
          {items.length === 0 ? (
            <div style={{ color: "var(--muted)", fontSize: 13 }}>{t("sync.empty")}</div>
          ) : (
            <>
              ...
              {needDecision === 0 && auto.length === 0 ? (
                <div style={{ color: "#5fd08a", fontSize: 13 }}>{t("sync.allSynced")}</div>
              ) : null}
            </>
          )}
        </>
